...
/**********************************************************************/
//Description:		WifiPairWifiMain   								
//Parameters:
//Return:
//Date:  
//author:		 	quanwu.xu
/**********************************************************************/
void WifiPairBlueMode(SystemApiTag *me, QEvent *e);

void WifiPairAPMode(SystemApiTag *me, QEvent *e)
{
	...
	switch(e->sig)
	{
		...
		//	TmrDeleteTimer(eTmrHdlFimewareUpgrade);
			StartFlag = TRUE;
		...
		case Q_EXIT_SIG:
			printflog("out, %s ,%d\r\n",__FUNCTION__,__LINE__);
			break;	
		case eSystemEventWifiPairStart:
			if(StartFlag == TRUE)
			{
		...
		case Q_eKeyEventKey0LongClik:
			printflog("APMode-2\n");
		    Q_TRAN(&WifiPairBlueMode);
			break;

		case eSystemEventWifiPairSuccess:
		//	mcu_set_wifi_mode(AP_CONFIG);	
			FlashSeteUpgradeFlag(eAPMode);
			WifiAO.eUpgradeFlag = eAPMode;
			BeepPollingHandle(100); 
			#if 0
			TmrInsertTimer(eTmrHdlWifiSendTimeout, TMR_PERIOD(300), NULL);
			Smoke_all_data_update();
			WifiAO.WifiPiredMode = eInitWifiPairMode;
			E2promWrite(FLASH_ADDR_WIFI_PAIRED_MODE, WifiAO.WifiPiredMode);
			SysAO.eMode = NormalMode;
			#endif
			break;
		case eSystemEventWifiPairFail:
			WifiDeinit();
		//	SysAO.eMode = NormalMode;
			break;
...
void WifiPairBlueMode(SystemApiTag *me, QEvent *e)
{
	...
	switch (e->sig)
	{
		...
		//	SystemPostFifo(&MainEvent); 
			Q_TRAN(&WifiPairAPMode);   
		//	SysAO.eMode = WifiPairWifiMode;
			printflog("EZMode-2\n");
			break;
		...
		case eSystemEventWifiPairSuccess:
			...
			//TmrInsertTimer(eTmrHdlWifiSendTimeout, TMR_PERIOD(300), NULL);
			//Smoke_all_data_update();
			//WifiAO.WifiPiredMode = eInitBluePairMode;
			//E2promWrite(FLASH_ADDR_WIFI_PAIRED_MODE, WifiAO.WifiPiredMode);
			//SysAO.eMode = NormalMode;
			break;
		case eSystemEventWifiPairFail:
...
void WiFiPirHandl(SystemApiTag *me,QEvent *e)
{
	switch (e->sig)
	{
		...
		case Q_eKeyEventKey0LongClik: //长按配网键
	#elif (defined SoftWareVer_SC240_WT_EN) 
		case Q_eKeyEventKey1LongClik:  //long test key
	#endif
